"""
DominoEX Varicode Encoding/Decoding

This module implements the varicode encoding used by DominoEX modes.
DominoEX varicode is a variable-length encoding using 1-3 nibbles (4-bit symbols).
Each character is represented by 1-3 nibbles, where continuation nibbles have
the MSB (bit 3) set to 1.

Based on fldigi's implementation:
...
# DominoEX varicode table
# Format: [nibble1, nibble2, nibble3]
# First 256 entries are primary alphabet, next 256 are secondary alphabet
# nibble2 and nibble3 are 0 if not used, or have MSB set if they are continuation nibbles
_VARICODE_TABLE = [
    # Primary alphabet (0-255)
...
def encode(text, secondary=False):
    ...
    Args:
        ...
        secondary: If True, use secondary alphabet (for secondary text channel)

    Returns:
        List of nibbles (0-15) representing the varicode
    ...
    """
    nibbles = []
    offset = 256 if secondary else 0

    for char in text:
        code = ord(char)
        if code > 255:
            code = ord('?')  # Replace unknown characters with '?'

        varicode = _VARICODE_TABLE[code + offset]
        nibbles.append(varicode[0])

        # Add continuation nibbles (those with MSB set, i.e., >= 8)
        if varicode[1] >= 8:
            nibbles.append(varicode[1])
            if varicode[2] >= 8:
                nibbles.append(varicode[2])
...
def encode_char(char, secondary=False):
    ...
    Args:
        char: Character to encode (single character string or int ASCII code)
        secondary: If True, use secondary alphabet

    Returns:
        List of 1-3 nibbles representing the character

    Example:
        >>> encode_char('A')
        [3, 9]
        >>> encode_char(65)  # ASCII for 'A'
        [3, 9]
    """
    offset = 256 if secondary else 0
    ...
    if varicode[1] >= 8:
        result.append(varicode[1])
        if varicode[2] >= 8:
            result.append(varicode[2])
