...
3. **Sub-Bin Estimation** - Refine frequency estimate using advanced estimators
...
- **Accuracy**: ±0.05-0.15 Hz with multi-estimator
...
```
✓ Detected: 1499.94 Hz (target: 1500 Hz, error: 0.06 Hz)
✓ Decoded: "Hello World!" (perfect)
```

### Frequency Estimators

Multiple estimators available with different accuracy/speed tradeoffs:

| Estimator | Accuracy | Speed | Use Case |
|-----------|----------|-------|----------|
...
| **multi** | **±0.08 Hz** | **★★★★☆** | **Best overall (default)** |

See [Frequency Estimation Guide](frequency-estimation.md) for details.
...
for i, peak in enumerate(peaks):
    ...
    decoded_texts[i] = []
    decoder.set_text_callback(lambda text, idx=i: decoded_texts[idx].append(text))
    decoders[i] = decoder
...
for i, peak in enumerate(peaks):
    ...
    print(f"Signal at {peak.frequency:.1f} Hz: {text}")
```
...
python tests/test_multi_psk.py
```
...
### Accuracy
- **Frequency Detection**: ±0.05-0.15 Hz with multi-estimator
...
### For Single Signal
```python
# Use auto-detection + AFC for best results
...
```python
# Enable squelch to prevent cross-talk
for peak in peaks:
    decoder = PSKDecoder(
        baud=125,
        frequency=peak.frequency,
        squelch_enabled=True  # IMPORTANT!
    )
```

### For Real-Time Monitoring
```python
# Use MultiSignalDetector for continuous tracking
tracker = MultiSignalDetector(
    sample_rate=8000,
    max_signals=5,
    update_interval=4000  # 0.5 seconds at 8 kHz
)
...
while True:
    ...
    tracker.update(audio_chunk)
    active = tracker.get_active_signals()
    # Update decoders as needed
```

## See Also

- [Frequency Estimation Guide](frequency-estimation.md) - Advanced frequency detection methods
- [PSK Decoder Guide](decoders.md) - Complete decoder user guide
- [API Reference](../api/reference/psk_decoder.md) - Full API documentation
